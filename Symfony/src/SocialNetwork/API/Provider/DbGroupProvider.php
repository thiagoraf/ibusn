...
use SocialNetwork\API\Interfaces\IGroupProvider,
    SocialNetwork\API\Entity\Group,
    SocialNetwork\API\Entity\GroupMember,
...
class DbGroupProvider implements IGroupProvider{
    ...
    {
        $group = $this->dbService->find('SocialNetwork\API\Entity\Group', $id);
        ...
        $this->dbService->createQueryBuilder()
            ->delete('SocialNetwork\API\Entity\GroupMember', 'gm')
            ->where('gm.groupUid = ?1')
            ->setParameter(1 , $id )
            ->getQuery()
            ->getResult();
    ...
    public function get($id, $attributes = array())
    {
        $group = $this->dbService->createQueryBuilder()
            ...
            ->from('SocialNetwork\API\Entity\Group', 'g')
            ->where('g.id = ?1')
            ->setParameter(1 , $id )
            ->getQuery()
            ->getArrayResult();
    ...
    public function setUserGroup($user, $group)
    {
        $groupMember = $this->dbService->createQueryBuilder()
            ->select( 'gm' )
            ->from('SocialNetwork\API\Entity\GroupMember', 'gm' )
    ...
    public function getUserGroups($user , array $attributes = array())
    {
        $result = $this->dbService->createQueryBuilder()
            ->select( MAP::mapDbAttributes('g' , $attributes) )
            ->from('SocialNetwork\API\Entity\Group', 'g')
            ->join('SocialNetwork\API\Entity\GroupMember', 'gm', 'WITH', "gm.groupUid = g.uid")
            ->where('gm.memberUid = ?1')
            ->setParameter(1 , $user )
            ->getQuery()
            ->getArrayResult();
